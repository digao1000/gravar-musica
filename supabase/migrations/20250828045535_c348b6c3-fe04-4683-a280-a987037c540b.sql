...
CREATE OR REPLACE FUNCTION public.get_orders_for_staff()
 RETURNS TABLE(id uuid, cliente_nome text, cliente_contato_masked text, pendrive_gb integer, status text, forma_pagamento text, observacoes text, total_gb real, total_itens integer, total_musicas integer, total_valor real, created_at timestamp with time zone, updated_at timestamp with time zone)
 LANGUAGE sql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- 2. Remove insecure public order creation policies
DROP POLICY IF EXISTS "Allow legitimate order creation" ON public.pedidos;
DROP POLICY IF EXISTS "Allow legitimate order item creation" ON public.pedido_itens;

-- 3. Create secure order creation function for public use
...
-- 4. Create secure policy for public order creation via function
CREATE POLICY "Allow public order creation via function" ON public.pedidos
FOR INSERT 
WITH CHECK (false); -- Block direct inserts, force use of function

CREATE POLICY "Allow public order item creation via function" ON public.pedido_itens  
FOR INSERT 
WITH CHECK (false); -- Block direct inserts, force use of function